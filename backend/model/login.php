  ...
  function attemptLogin($user, $pass){
    ...
    if(COUNT($results) === 1){ 
      if(password_verify($pass, $results[0]['password'])){
        $validatedUser = $results[0]['user_id'];
...
return FALSE;
  ...
  public function validateToken($user){
    try {
      ...
      $token = json_decode($user, true); 
      $JWT = $token['jwt']; 
      $DecodedDataArray = JWT::decode($JWT, $secretKey, array(ALGORITHM));
      $decodedToken = json_decode($token, true);
    
      $fName = $decodedToken['data']['fName'];
      $lName = $decodedToken['data']['lName'];
      $email = $decodedToken['data']['email'];
      $validatedUser = $decodedToken['data']['user_id']; 
      $user_type_id = $decodedToken['data']['user_type']['user_type_id'];
      $name = $decodedToken['data']['user_type']['name'];
